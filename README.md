...
Package repositories call [the shared workflow](.github/workflows/package.yml) with an explicit Norm toolchain version. It resolves the declaration, rejects uncommitted resolution changes, packages one root Module, runs every `Main.norm`, attests tagged artifacts, and publishes the immutable Release.
